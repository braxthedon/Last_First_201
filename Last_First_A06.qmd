...
```{r}
spread<- abs(bears-opponent)
largest_spread<- spread[which.max(spread)]
paste("The largest win spread is", largest_spread,", when the Bears scored", bears[which.max(spread)], "points and the Opponent scored", opponent[which.max(spread)], "points.")
```
...
```{r}
number_of_factors <- function(int){
  num_fact = 0
  for (i in 1:int){
    if (int %% i==0){
      num_fact = num_fact+1
...
```{r}
area_func <- function(l,w){
  return(l*w)
}
area<- area_func(102,60)
paste("The area in square inches is", area)
paste("The area in square feet is", area/144)
```
...
```{r}
max_letters<- function(str_vect){
  letters_in_word <- nchar(str_vect)
  max_letters <- max(letters_in_word)
  longest_words<- str_vect[letters_in_word == max_letters]
...
is_in_vector <- function(val,vec){
  if (val %in% vec){
    paste(val, "is in the vector.")
  }else{
    paste(val, "is not in the vector.")
  }
}
is_in_vector(value, check)
```
